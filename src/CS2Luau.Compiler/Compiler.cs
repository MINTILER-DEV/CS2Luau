...
{
    private const string EnumWrapperMetadataName = "Roblox.Enum";
    private const string ImplicitUsingsSource =
        ...
        global using Enum = global::Roblox.Enum;
        """;
    ...
    public Task<CompileResult> CompileAsync(CompileInput input, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var tree in compilation.SyntaxTrees.Where(tree => !IsImplicitSyntaxTree(tree)))
        {
    ...
    private static CSharpCompilation CreateCompilation(CompileInput input)
    {
        ...
        var syntaxTrees = input.Documents
            ...
            .Concat(GetFallbackEnumWrapperSyntaxTrees(parseOptions))
            .ToArray();
    ...
    private static IEnumerable<SyntaxTree> GetFallbackEnumWrapperSyntaxTrees(CSharpParseOptions parseOptions)
    {
        var robloxAssembly = typeof(global::Roblox.Globals).Assembly;
        if (robloxAssembly.GetType(EnumWrapperMetadataName, throwOnError: false, ignoreCase: false) is not null)
        {
            return [];
        }

        return [CSharpSyntaxTree.ParseText(BuildFallbackEnumWrapperSource(robloxAssembly), parseOptions, "<implicit-roblox-enum-wrapper>")];
    }

    private static string BuildFallbackEnumWrapperSource(Assembly robloxAssembly)
    {
        var builder = new StringBuilder();
        builder.AppendLine("// <auto-generated />");
        builder.AppendLine("#nullable enable");
        builder.AppendLine();
        builder.AppendLine("namespace Roblox;");
        builder.AppendLine();
        builder.AppendLine("public static class Enum");
        builder.AppendLine("{");

        foreach (var enumType in GetLoadableTypes(robloxAssembly)
                     .Where(type => type.IsEnum && string.Equals(type.Namespace, "Roblox.Enums", StringComparison.Ordinal))
                     .OrderBy(type => type.Name, StringComparer.Ordinal))
        {
            builder.AppendLine($"    public static class {enumType.Name}");
            builder.AppendLine("    {");

            foreach (var memberName in System.Enum.GetNames(enumType))
            {
                builder.AppendLine($"        public static readonly global::Roblox.Enums.{enumType.Name} {memberName} = global::Roblox.Enums.{enumType.Name}.{memberName};");
            }
